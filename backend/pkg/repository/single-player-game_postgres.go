...
func (s *SinglePlayerGamePostgres) SendResults(lc entities.LevelComplete, totalPush int, totalErr int) error {
	...
	if levelInfo.Type == "classic" {
		...
		stats.NumClassesClassic[entities.GetClassIndexByAccuracy(lc.Accuracy)] += 1
...
func (s *SinglePlayerGamePostgres) getLevel(levelID int) (entities.Level, error) {
	var result entities.Level

	//var query = fmt.Sprintf("SELECT * FROM Level WHERE id = %s", cast.ToString(levelID))
...
func (s *SinglePlayerGamePostgres) castToJSON1(statsDB entities.PlayerStatsDB) (entities.PlayerStats, error) {
