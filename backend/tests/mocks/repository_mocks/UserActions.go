...
// DropRefreshToken provides a mock function with given fields: id, newTime
func (_m *UserActions) DropRefreshToken(id int, newTime time.Time) (int, error) {
	ret := _m.Called(id, newTime)
	...
	if rf, ok := ret.Get(0).(func(int, time.Time) (int, error)); ok {
		return rf(id, newTime)
	}
	if rf, ok := ret.Get(0).(func(int, time.Time) int); ok {
		r0 = rf(id, newTime)
	} else {
		r0 = ret.Get(0).(int)
	}
...
// UpdateAvatarPath provides a mock function with given fields: id, newPath
func (_m *UserActions) UpdateAvatarPath(id int, newPath string) (string, error) {
	ret := _m.Called(id, newPath)

	if len(ret) == 0 {
		panic("no return value specified for UpdateAvatarPath")
	}

	var r0 string
	var r1 error
	if rf, ok := ret.Get(0).(func(int, string) (string, error)); ok {
		return rf(id, newPath)
	}
